    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        half_letter = letter_thickness/2
        h1_corner = rg.Point(intersection_center.x - (width/2), intersection_center.y - half_letter)
        h2_corner = rg.Point(intersection_center.x + (width/2), intersection_center.y + half_letter)
        self.h_rect = rg.Rectangle(h1_corner, h2_corner)
        v1_corner = rg.Point(intersection_center.x - half_letter, intersection_center.y - half_letter)
        v2_corner = rg.Point(intersection_center.x + half_letter, intersection_center.y + height - half_letter)
        self.v_rect = rg.Rectangle(v1_corner, v2_corner)
    ...
    def clone(self):
        ...
        x_center = (width / 2) + self.h_rect.corner_1.x
        y_center = (thickness / 2) + self.h_rect.corner_1.y
        center = rg.Point(x_center, y_center)
        new = CapitalT(center, width, height, thickness)
